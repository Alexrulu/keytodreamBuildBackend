  ...
  return (
    <>
      ...
      <div className="w-full my-3 px-3 flex flex-col gap-5 md:px-10 lg:px-20 lg:grid lg:grid-cols-3">
        ...
        {properties.filter(property => property.type === 2).map((property, index) => (
          <Link to={`/article/${property.id}`} key={property.id}>
            <article className="bg-zinc-100 h-[20vh] w-full justify-between flex gap-1 text-zinc-800 text-sm hover:scale-105 duration-300">
